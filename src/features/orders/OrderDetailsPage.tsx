...
export const OrderDetailsPage = ({ navItems, utilityItems, shellTitle }: OrderDetailsPageProps) => {
  const session = useSessionStore((state) => state.session);
  const { id } = useParams();
  const navigate = useNavigate();

  const { data: order, loading, error } = useLiveQuery(
    async () => (id ? getOrderById(id) : Promise.resolve(undefined)),
    undefined,
  ...
  const content = loading ? (
    ...
    <EmptyState title="داواکاری نەدۆزرایەوە" description="ڕەنگە سڕابێتەوە یان ناسنامەکە هەڵە بێت." />
  ) : (
    <OrderDetailsPanel
      order={order}
      backAction={
  ...
  return (
    <DashboardShell title={shellTitle} subtitle="چاپ و شوێنکەوتنی وردەکاری" navItems={navItems} utilityItems={utilityItems}>
      {content}
    </DashboardShell>
  );
};
