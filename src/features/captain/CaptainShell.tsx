import { Bell, ClipboardList, Clock3, FileText, House, Info, LogOut, Settings2, Truck, type LucideIcon } from 'lucide-react';
...
export const CaptainShell = ({ children }: { children: ReactNode }) => {
  ...
  const pageConfig: Array<{ match: (pathname: string) => boolean; label: string; icon: LucideIcon }> = [
    { match: (pathname) => pathname === '/captain', label: 'سەرەکی', icon: House },
    { match: (pathname) => pathname === '/captain/orders' || pathname === '/captain/delivery', label: 'داواکاریەکان', icon: ClipboardList },
    { match: (pathname) => pathname === '/captain/history', label: 'مێژوو', icon: Clock3 },
    { match: (pathname) => pathname === '/captain/notifications', label: 'ئاگەدارکردنەوەکان', icon: Bell },
    { match: (pathname) => pathname === '/captain/about', label: 'دەربارە', icon: Info },
    { match: (pathname) => pathname === '/captain/settings', label: 'ڕێخستن', icon: Settings2 },
  ...
  const isOrdersSection =
    location.pathname === '/captain/orders' ||
    location.pathname === '/captain/delivery' ||
    location.pathname.startsWith('/orders/') ||
    location.pathname.startsWith('/delivery-orders/');
  const isHistorySection = location.pathname === '/captain/history';
  const isAboutSection = location.pathname === '/captain/about';
  const isSettingsSection = location.pathname === '/captain/settings';
            ...
            <div className="flex items-center gap-3 overflow-hidden" dir="ltr">
              ...
              <div className="flex min-w-0 flex-1 justify-end overflow-x-auto pb-1 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
                <div className="flex shrink-0 flex-row-reverse flex-nowrap items-center gap-1.5">
                  <NavLink end to="/captain" className={({ isActive }) => navClassName(isActive)} title="سەرەکی" aria-label="سەرەکی">
                  ...
                  <NavLink
                    ...
                    title="داواکاریەکان"
                    aria-label="داواکاریەکان"
                  >
                    <ClipboardList className="h-5 w-5" />
                  </NavLink>
                  <NavLink
                    to="/captain/history"
                    className={() => navClassName(isHistorySection)}
                    title="مێژوو"
                    aria-label="مێژوو"
                  >
